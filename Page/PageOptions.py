...
class ChromeSearchLocators:
    ...
    CHROME_LOCATOR_FIND_BUTTON = (
        By.XPATH,
        '//input[@class="el_18 submit nice_button"]',
    )

    def country(self, title):
        chrome_locator_search_country = (
            By.XPATH,
            f'//div//select[@id="country"]/option[@value=contains(text(),"{title}")]',
        )
        return chrome_locator_search_country

    def genre(self, title):
        chrome_locator_search_genre = (
            By.XPATH,
            f'//div//select[@id="m_act[genre]"]/option[@value=contains(text(), "{title}")]',
        )
...
class SearchHelper(AdvancedSearchPage):
    ...
    def find_button(self):
        enter_button = self.find_element(
            ChromeSearchLocators.CHROME_LOCATOR_FIND_BUTTON
        )
        enter_button.click()
        return enter_button
